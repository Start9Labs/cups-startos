...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh cups-messenger/target/aarch64-unknown-linux-musl/release/cups manifest.yaml httpd.conf cups-messenger-ui/www
	...
	DOCKER_CLI_EXPERIMENTAL=enabled docker buildx build --tag start9/$(PKG_ID)/main:$(PKG_VERSION) --build-arg ARCH=aarch64 --platform=linux/arm64 -o type=docker,dest=docker-images/aarch64.tar .

cups-messenger/target/aarch64-unknown-linux-musl/release/cups: $(BACKEND_SRC)
	docker run --rm -it -v ~/.cargo/registry:/root/.cargo/registry -v "$(shell pwd)"/cups-messenger:/home/rust/src messense/rust-musl-cross:aarch64-musl cargo build --release
...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh cups-messenger/target/x86_64-unknown-linux-musl/release/cups manifest.yaml httpd.conf cups-messenger-ui/www
	mkdir -p docker-images
	DOCKER_CLI_EXPERIMENTAL=enabled docker buildx build --tag start9/$(PKG_ID)/main:$(PKG_VERSION) --build-arg ARCH=x86_64 --platform=linux/amd64 -o type=docker,dest=docker-images/x86_64.tar .

cups-messenger/target/x86_64-unknown-linux-musl/release/cups: $(BACKEND_SRC)
	docker run --rm -it -v ~/.cargo/registry:/root/.cargo/registry -v "$(shell pwd)"/cups-messenger:/home/rust/src messense/rust-musl-cross:x86_64-musl cargo build --release
